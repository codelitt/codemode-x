...
### Added
- **OpenAPI adapter supports remote URLs** — `source` field can now be an `http://` or `https://` URL. The adapter fetches the spec at parse time. Local file paths still work as before.
